...
import static org.hyland.sdk.cic.qna.mapper.QnaMapperUtils.writeSetAnswerReferences;
...
class SetAnswerRequestMapper implements CICMapper<SetAnswerRequest> {
    ...
    public CICNode toCICNode(SetAnswerRequest request) {
        ...
        if (!request.references().isEmpty()) {
            obj.putArray("references", writeSetAnswerReferences(request.references()));
        }
        return obj;
    }
}
