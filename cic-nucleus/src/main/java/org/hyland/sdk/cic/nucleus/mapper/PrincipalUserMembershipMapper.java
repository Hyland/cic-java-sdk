...
class PrincipalUserMembershipMapper implements CICMapper<PrincipalUserMembership> {
    ...
    static class PaginatedListMapper implements CICMapper<PrincipalUserMembership.PaginatedListOf> {

        private final PrincipalUserMembershipMapper innerMapper = new PrincipalUserMembershipMapper();

        @Override
        public PrincipalUserMembership.PaginatedListOf fromCICNode(CICNode cicNode) {
            if (!(cicNode instanceof CICObject obj)) {
                throw new IllegalArgumentException("Expected CICObject, got: " + cicNode.getClass().getSimpleName());
            }
            ...
            return new PrincipalUserMembership.PaginatedListOf(items, next);
        }
    }
}
